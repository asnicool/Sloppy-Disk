...
func TestNewFreeDBProvider(t *testing.T) {
	provider := NewFreeDBProvider()
	if provider == nil {
...
func TestFreeDBProvider_Name(t *testing.T) {
	provider := &FreeDBProvider{}
	if provider.Name() != "FreeDB" {
		t.Errorf("Expected name 'FreeDB', got '%s'", provider.Name())
	}
}

func TestFreeDBProvider_Search(t *testing.T) {
...
.`))
	...
	provider := NewFreeDBProvider()
	// Note: Cannot easily override URL in FreeDBProvider without modifying the struct
	// The test structure is valid but cannot execute without modifying provider
	_ = provider
}

func TestFreeDBProvider_GetReleaseDetails(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/~cddb/cddb.cgi" {
			t.Errorf("Expected path /~cddb/cddb.cgi, got %s", r.URL.Path)
		}
...
func TestFreeDBProvider_GetCoverArt(t *testing.T) {
	provider := NewFreeDBProvider()
	...
	if results != nil && len(results) != 0 {
		t.Errorf("Expected 0 cover art results from FreeDB, got %d", len(results))
	}
}

func TestFreeDBProvider_ParseSearchResponse(t *testing.T) {
	// Test parsing FreeDB search response format
...
func TestFreeDBProvider_ParseReleaseDetails(t *testing.T) {
	// Test parsing FreeDB release details response format
...
func TestFreeDBProvider_NewRequest(t *testing.T) {
	provider := NewFreeDBProvider()
